...
function ProjectAccessManager({ open, onClose, project }) {
  ...
  const fetchEngineers = async () => {
    try {
      ...
      const response = await api.get('/projects/engineers');
  ...
  const fetchAssignedEngineers = async () => {
    try {
      const response = await api.get(`/projects/${project._id}/assigned-engineers`);
      setAssignedEngineers(response.data);
      setSelectedEngineers(response.data?.map(eng => eng.username) || []);
    } catch (error) {
  ...
  const handleAssignEngineers = async () => {
    try {
      ...
      // Filter out already assigned engineers and get usernames
      const newEngineersToAssign = selectedEngineers.filter(
        username => !assignedEngineers.some(ae => ae.username === username)
      );

      await api.put(`/projects/${project._id}/assign-engineers`, {
        engineerUsernames: [...assignedEngineers.map(ae => ae.username), ...newEngineersToAssign]
      });
    ...
    const handleRemoveEngineer = async (engineerUsername) => {
    try {
      setLoading(true);
      setError('');

      // Remove engineer by updating the assignment with remaining engineers
      const remainingEngineers = assignedEngineers
        .filter(eng => eng.username !== engineerUsername)
        .map(eng => eng.username);

      await api.put(`/projects/${project._id}/assign-engineers`, {
        engineerUsernames: remainingEngineers
      });
  ...
  const getUnassignedEngineers = () => {
    const assignedUsernames = assignedEngineers.map(eng => eng.username);
    return engineers.filter(eng => !assignedUsernames.includes(eng.username));
  };
    ...
    >
              ...
              {assignedEngineers.map((engineer) => (
                ...
                >
                  <Box display="flex" alignItems="center" gap={2}>
                    ...
                    <Box>
                      <Typography variant="body1" fontWeight="medium">
                        <strong>{engineer.username}</strong> - {engineer.name}
                      </Typography>
                  ...
                  <Tooltip title="Remove from project">
                    <IconButton
                      color="error"
                      onClick={() => handleRemoveEngineer(engineer.username)}
                      size="small"
                    >
                      <PersonRemove />
                    </IconButton>
        ...
        <Box>
          ...
          {loadingEngineers ? (
            ...
            <FormControl fullWidth>
              ...
              <Select
                multiple
                value={selectedEngineers.filter(username => !assignedEngineers.some(ae => ae.username === username))}
                onChange={handleSelectionChange}
                input={<OutlinedInput label="Select Engineers by Username" />}
                MenuProps={MenuProps}
                renderValue={(selected) => (
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                    {selected.map((username) => {
                      const engineer = engineers.find(eng => eng.username === username);
                      return (
                        <Chip
                          key={username}
                          label={`${engineer?.username} (${engineer?.name})` || username}
                          size="small"
                          sx={{ m: 0.25 }}
                        />
                      );
              ...
              >
                {getUnassignedEngineers().map((engineer) => (
                  <MenuItem key={engineer.username} value={engineer.username}>
      ...
      <DialogActions sx={{ p: 3, pt: 1 }}>
        ...
        <Button
          ...
          variant="contained"
          disabled={loading || selectedEngineers.filter(username => !assignedEngineers.some(ae => ae.username === username)).length === 0}
